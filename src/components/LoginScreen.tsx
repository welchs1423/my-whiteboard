...
export default function LoginScreen({ isDarkMode, setIsDarkMode, nickname, setNickname, onJoin }: Props) {
  return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', backgroundColor: isDarkMode ? '#111827' : '#f3f4f6' }}>
      <div style={{ padding: '30px', backgroundColor: isDarkMode ? '#1f2937' : 'white', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)', display: 'flex', flexDirection: 'column', gap: '15px', width: '300px' }}>
        ...
        <button onClick={() => onJoin(true)} style={{ padding: '8px', backgroundColor: 'none', background: 'none', border: `1px solid ${isDarkMode ? '#374151' : '#d1d5db'}`, borderRadius: '6px', cursor: 'pointer', color: isDarkMode ? '#9ca3af' : '#6b7280', fontSize: '13px' }}>
          👁️ 보기만 하기 (읽기 전용)
        </button>
